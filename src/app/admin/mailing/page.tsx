...
import {
  AdicionarContato,
  ImportarLista,
} from "@/components/admin/mailing-formularios";
...
export default async function PaginaMailing({ searchParams }: Props) {
  ...
  if (dados === null) {
    return (
      <>
        <Cabecalho descricao="Nomes e e-mails para quem o escritório divulga as matérias publicadas." />
    ...
    <>
      <div className="flex flex-wrap items-end justify-between gap-5 sm:gap-6">
        <Cabecalho
          descricao={
            totalGeral === 0
              ? "A lista está vazia. Cole os nomes e e-mails abaixo para começar."
              : `${totalGeral} ${totalGeral === 1 ? "contato" : "contatos"} guardados · ${ativos} recebendo os envios. Contato desligado continua cadastrado, mas fica de fora.`
          }
        />
        ...
        {totalGeral > 0 ? (
          <a
            ...
            className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-[2px] border border-bordo-700 px-5 text-[0.9375rem] font-medium text-bordo-700 transition-colors hover:bg-bordo-700 hover:text-white sm:w-auto"
          >
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-areia-200 pb-4">
            ...
            >
              <label htmlFor="q" className="sr-only">
                Buscar por nome, e-mail ou origem
              </label>
